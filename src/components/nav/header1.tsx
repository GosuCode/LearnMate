...
const semesters = [
  ...
  },
  {
    label: "MORE",
    subjects: [
      { name: "BCA Syllabus", href: "/bca-syllabus" },
      { name: "BCA Notice", href: "/notice" },
    ],
  },
];
          ...
          <div className="flex flex-wrap gap-3">
            {semesters.map((sem) => (
              <DropdownMenu
                key={sem.label}
                onOpenChange={(open) =>
                  setActiveDropdown(open ? sem.label : null)
                }
              >
                ...
                <DropdownMenuContent className="bg-card border border-border shadow-lg">
                  {sem.subjects.map((subj) => (
                    <DropdownMenuItem
                      key={subj.href}
                      asChild
                      className="!cursor-pointer hover:!bg-accent focus:!bg-primary focus:!text-primary-foreground"
                    >
                      <Link
                        href={subj.href}
